# Generative Methods for Constructing MRI-Forensics Dataset


## Models Trained
We trained the following generative models using their official code repositories:

### Generative Adversarial Networks (GANs)
1. **ProGAN**  
   Karras et al., "Progressive Growing of GANs for Improved Quality, Stability, and Variation," 2017. [Paper](https://arxiv.org/abs/1710.10196) • [Github](https://github.com/tkarras/progressive_growing_of_gans)
   
3. **StyleGANv2-ADA**  
   Karras et al., "Training Generative Adversarial Networks with Limited Data," 2020. [Paper](https://arxiv.org/abs/2006.06676) • [Github](https://github.com/NVlabs/stylegan2-ada-pytorch))

### Diffusion Model
3. **DiT (Diffusion Transformer)**  
   Peebles et al., "Scalable Diffusion Models with Transformers," 2023. [Paper](https://arxiv.org/abs/2212.09748) • [Github](https://github.com/facebookresearch/DiT)

### Autoregressive Generative Model
4. **RAR (Randomized Autoregressive Generation)**  
   Yu et al., "Randomized autoregressive visual generation," 2024. [Paper](https://doi.org/10.48550/arXiv.2411.00776) • [Github](https://github.com/bytedance/1d-tokenizer)

---

### MRI-Forensics Dataset Access

The dataset is hosted on Google Drive and can be accessed here:
[Download MRI-Forensics Dataset](https://drive.google.com/drive/folders/1e8EJVNTdYWAuSU2V7Sil6YBNalbJoapb?usp=drive_link)

> **License:** By downloading this dataset, you agree to the [Creative Commons Attribution-NonCommercial 4.0 International License](https://creativecommons.org/licenses/by-nc/4.0/).

The dataset consists of seven folders:
1. One folder contains real prostate MRI images owned by the authors.
2. The remaining six folders include synthesized prostate MRI generated by advanced generative models.

> **Note:** A separate Brain MRI dataset used in the study is available upon request to the authors.
---

## Spectral Fingerprint Analysis of Generated Images

To analyze the frequency characteristics of generated MRI images, we include a script to compute the **average power spectrum**.

This is adapted from the paper:  
[Intriguing Properties of Synthetic Images: From Generative Adversarial Networks to Diffusion Models](https://doi.org/10.48550/arXiv.2304.06408).

### Example Usage

```bash
python spectra.py --files_path ./MRI-Forensics/Folder_1 --out_dir ./output --out_name name_of_generative_model
```  
This will save the power spectrum plot fft2_gray.png in ./output/name_of_generative_model/
> Note: This adaptation is for academic, non-commercial use only, in accordance with the original license.

